...
router.put('/:universityId', async (req, res) => {
    try {
        const { name, location, description } = req.body;
        const query = "UPDATE universities SET name = $1, location = $2, description = $3 WHERE id = $4 RETURNING *";
...
router.delete('/:universityId', async (req, res) => {
    try {
        const results = await db.query("DELETE FROM universities WHERE id = $1", [req.params.universityId]);
